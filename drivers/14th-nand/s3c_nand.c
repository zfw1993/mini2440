...
static int s3c_nand_init(void)
{
	...
	st_p_nand_chip->ecc.mode    = NAND_ECC_NONE;    
...
#define TWRPH1  0
	...
	/* 注意用上面这个函数，无法创建出块设备*/
	//nand_scan(st_p_mtd, 1); /* 识别nand flash, 构造mtd_info */
	res = nand_scan_ident(st_p_mtd, 1, NULL);
		
	if (res == 0) {
	//	s3c2410_nand_update_chip(info, nmtd);  /* 不需要做一步，我们用软件生成ECC码 */
		nand_scan_tail(st_p_mtd);	
	//	mtd_device_register(st_p_mtd, s3c_nand_parts, 4);
	}

	/* 5. add_mtd_partitions      */  /* 以上程序是识别出nand flash的 */
	                                  /* 而add_mtd_partitons是通知字符设备或块设备，分别做相应的操作的 */
	//add_mtd_partitions(st_p_mtd, s3c_nand_part, 4); 

	mtd_device_parse_register(st_p_mtd, NULL, NULL, /* 分区 */
					 s3c_nand_parts, 4);
...
{
	mtd_device_unregister(st_p_mtd);
	kfree(st_p_mtd);
